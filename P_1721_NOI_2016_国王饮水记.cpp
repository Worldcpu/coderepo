...
#include<bits/stdc++.h>
using namespace std;
constexpr int MN=8520;
struct Node{
    double x,y;
}q[MN];
int n,K,p,lim,que[MN],a[MN],h[MN],sum[MN],g[MN][20];
double f[MN][20];
...
Decimal solve(int x,int y){
    if(!y) return h[1];
    return (solve(g[x][y],y-1)+sum[x]-sum[g[x][y]])/(x-g[x][y]+1);
}
...
int main(){
    ...
    for(int j=1;j<=lim;j++){
        ...
        que[1]=1;
        for(int i=1;i<=n;i++){
            q[i]={1.0*i-1,sum[i]-f[i][j-1]};
        }       
        for(int i=2;i<=n;i++){
            Node u={1.0*i,sum[i]};
            while(ql<qr&&getk(u,q[que[ql]])<getk(u,q[que[ql+1]])) ql++;
            g[i][j]=que[ql];
            f[i][j]=(f[que[ql]][j-1]+sum[i]-sum[que[ql]])/(i-que[ql]+1);
            while(ql<qr&&getk(q[que[qr-1]],q[que[qr]])>getk(q[que[qr]],q[i])) qr--;
            que[++qr]=i;
        }
    }
    int u=n-K+lim,ret=0;
    for(int i=0;i<=lim;i++){
        if(f[u][i]>f[u][ret]) ret=i;
    }
    Decimal ans=solve(u,ret);
