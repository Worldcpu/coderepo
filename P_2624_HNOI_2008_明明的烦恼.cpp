/* 
  BigInteger.h (Version 2.7 Simplified)
...
__int128 BigInteger::to_int128() const {
	__int128 res = 0;
	for (int i = size; i >= 1; i--) res = res * BASE + digits[i];
	return res;
}
#endif
...
int n,dg[MN],sumd,cnt;;

int main(){
    cin>>n;
    for(int i=1;i<=n;i++){
        cin>>dg[i];
        if(~dg[i]) sumd+=dg[i]-1,cnt++;
    }
    if(sumd>n-2){
        cout<<0;
        return 0;
    }
    BigInteger ans1=factorial(sumd);
    for(int i=1;i<=n;i++){
        if(~dg[i]){
            ans1=ans1/factorial(dg[i]-1);
        }
    }
    BigInteger ans=factorial(n-2)/factorial(sumd)*ans1*(BigInteger(n-cnt).pow(n-2-sumd));
    cout<<ans;
