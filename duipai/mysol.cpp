...
using namespace std;
constexpr int MN=1e6+15,MV=1e5+15,MQ=5e5+15,INF=1e18;
struct Query{
    int op,l,r,x;
}qry[MN];
int n,m,a[MN],ans[MN],b[MN],pos[MN];
vector<int> pz;
...
namespace DSU{
    ...
    void merge(int x,int y){ // merge x -> y on |V|
        if(pos[y]){
            pos[y]=pos[x];
            b[pos[y]]=y;
            siz[y]+=siz[x];
...
namespace FKOP{
    ...
    void init(int id,int L,int R,bool ck=1){
        cleardsu(L,R);
        bmx=-INF;
        tag=0;
        if(ck){
    ...
    void bj(int fl,int fr,int id,int x){
    ...
    int query(int fl,int fr,int id,int x){
        int L=(id-1)*blen+1,R=min(id*blen,n);
